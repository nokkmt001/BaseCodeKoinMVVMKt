...
import com.phat.testbase.devphat.utils.ContainsUtils.AEON_API_KEY
import com.phat.testbase.devphat.utils.ContainsUtils.DEFAULT_LANGUAGE
import com.phat.testbase.devphat.utils.ContainsUtils.HEADER_HEAD_TOKEN
...
abstract class BaseMvvmViewModel : BindingViewModel() {
    ...
    protected open fun getHeaders(token: String?): HashMap<String, Any> {
        ...
        if (token != null && !HEADER_HEAD_TOKEN.isEmpty())
        ...
        if (ContainsUtils.isMultiLanguage) headers[ContainsUtils.HEADER_LANG] = DEFAULT_LANGUAGE
        headers[ContainsUtils.HEADER_API_KEY] = AEON_API_KEY
